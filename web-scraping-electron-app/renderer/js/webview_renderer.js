// Get the ipcRenderer object exposed by the preload script
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Reload the current webview URL
    $('#reloadWebviewBtn').on('click', () => {
        const currentUrl = $('#ext-url-window').attr('src');
        if (currentUrl && currentUrl !== 'about:blank') {
            $('#ext-url-window').attr('src', currentUrl);
            console.log('Reloaded webview with URL:', currentUrl);
        } else {
            alert('No URL to reload!');
        }
    });
});
