...
const require$$1 = require("querystring");
...
function getOsVersion() {
  var osName = os$3.type().replace("_", " ");
  var osVersion = os$3.release();
  if (osName === "Darwin") {
    osName = "macOS";
...
function getMacOsVersion() {
  var release = Number(os$3.release().split(".")[0]);
  return "10." + (release - 4);
}
function isDev$1() {
  var app = getApp();
  if (app && app.isPackaged !== void 0) {
    return !app.isPackaged;
  }
  if (typeof process.execPath === "string") {
    var execFileName = path$6.basename(process.execPath).toLowerCase();
...
function isIpcChannelListened(channel) {
  var ipc2 = getIpc();
  return ipc2 ? ipc2.listenerCount(channel) > 0 : false;
}
...
function openUrl(url2, logFunction) {
  logFunction = logFunction || console.error;
  var shell = getElectronModule("shell");
  if (!shell) return;
  shell.openExternal(url2).catch(logFunction);
}
var electronApi$2 = electronApi$3;
var queryString = require$$1;
var isAttached = false;
var catchErrors = function catchErrors2(options) {
  if (isAttached) return { stop };
  isAttached = true;
  ...
  function onError(e) {
    try {
      if (typeof options.onError === "function") {
        var versions = electronApi$2.getVersions();
  ...
  function createIssue(pageUrl, queryParams) {
    ...
    electronApi$2.openUrl(issueUrl, options.log);
  }
};
var log_1 = {
  compareLevels,
...
function log$3(electronLog, options) {
...
function scopeFactory(electronLog) {
  scope2.labelPadding = true;
  scope2.defaultLabel = "";
  scope2.maxLabelLength = 0;
  scope2.getOptions = getOptions;
  return scope2;
  function scope2(label) {
    ...
    electronLog.levels.forEach(function(level) {
      instance[level] = log$2.bind(null, electronLog, {
        level,
        scope: instance
      });
    });
  ...
  function getOptions() {
    return {
      defaultLabel: scope2.defaultLabel,
      labelLength: getLabelLength()
    };
  }
  function getLabelLength() {
    if (scope2.labelPadding === true) {
      return scope2.maxLabelLength;
    }
    if (scope2.labelPadding === false) {
      return 0;
    }
    if (typeof scope2.labelPadding === "number") {
      return scope2.labelPadding;
    }
    return 0;
  }
}
var console$1 = { exports: {} };
...
function maxDepth(data, depth) {
  ...
  if (depth < 1) {
    if (isArray$1(data)) return "[array]";
...
function toStringFactory(inspectOptions) {
  return function toStringFunction(data) {
    ...
    if (util$5.formatWithOptions) {
      simplifiedData.unshift(inspectOptions || {});
      return util$5.formatWithOptions.apply(util$5, simplifiedData);
    }
    return util$5.format.apply(util$5, simplifiedData);
  };
}
...
function styleToAnsi(style2) {
  var color = style2.replace(/color:\s*(\w+).*/, "$1").toLowerCase();
...
function removeStyles(data) {
  return transformStyles(data, function() {
    return "";
  });
}
...
function templateDate(data, message) {
  var template2 = data[0];
  if (typeof template2 !== "string") {
    return data;
  }
  data[0] = formatDate(template2, message.date);
  return data;
}
...
function templateVariables(data, message) {
  var template2 = data[0];
  var variables2 = message.variables;
  if (typeof template2 !== "string" || !message.variables) {
    return data;
  }
  for (var i in variables2) {
    if (!Object.prototype.hasOwnProperty.call(variables2, i)) continue;
    template2 = template2.replace("{" + i + "}", variables2[i]);
  }
  template2 = template2.replace("{level}]", padString(message.level + "]", 6));
  template2 = template2.replace("{level}", message.level);
  data[0] = template2;
  return data;
}
function templateText(data) {
  ...
  var textTplPosition = template2.lastIndexOf("{text}");
  ...
  var templatePieces = template2.split("{text}");
  var result = [];
  if (templatePieces[0] !== "") {
...
function customFormatterFactory(customFormat, concatFirst, scopeOptions) {
  ...
  if (typeof customFormat === "function") {
    return function customFunctionFormatter(data, message) {
      var modifiedMessage = Object.assign({}, message, { data });
...
function transform$4(message, transformers, initialData) {
...
console$1.exports = consoleTransportFactory;
console$1.exports.transformRenderer = transformRenderer;
console$1.exports.transformMain = transformMain;
...
function transformRenderer(message, transport, scopeOptions) {
  return transform$3.transform(message, [
    transform$3.customFormatterFactory(transport.format, true, scopeOptions)
  ]);
}
function transformMain(message, transport, scopeOptions) {
  ...
  return transform$3.transform(message, [
    ...
    transform$3.customFormatterFactory(transport.format, false, scopeOptions),
    useStyles ? transform$3.applyAnsiStyles : transform$3.removeStyles,
    transform$3.concatFirstStringElements,
    transform$3.maxDepthFactory(4),
    transform$3.toJSON
  ]);
}
...
function canUseStyles(useStyleValue, level) {
  ...
  var stream2 = useStderr ? process.stderr : process.stdout;
...
var consoleExports = console$1.exports;
var EventEmitter = require$$0$1;
var fs$4 = require$$0$2;
var os$2 = require$$2;
var path$5 = path$7;
var url$2 = require$$0$3;
var util$4 = require$$1$1;
var file$1 = {
  FileRegistry: FileRegistry$1
};
...
util$4.inherits(File, EventEmitter);
...
File.prototype.getSize = function() {
  if (this.initialSize === void 0) {
    try {
      var stats = fs$4.statSync(this.path);
...
File.prototype.nextAsyncWrite = function() {
  ...
  fs$4.writeFile(this.path, text, this.writeOptions, function(e) {
    file2.hasActiveAsyncWritting = false;
    if (e) {
      file2.emit(
        "error",
...
NullFile.prototype.getSize = function() {
  return 0;
};
NullFile.prototype.isNull = function() {
  return true;
};
function FileRegistry$1() {
  EventEmitter.call(this);
  this.store = {};
  this.emitError = this.emitError.bind(this);
}
util$4.inherits(FileRegistry$1, EventEmitter);
FileRegistry$1.prototype.provide = function(filePath, writeOptions, async2) {
  var file2;
  try {
    ...
    file2 = this.createFile(filePath, writeOptions, Boolean(async2));
  } catch (e) {
    file2 = new NullFile(filePath);
...
FileRegistry$1.prototype.createFile = function(filePath, writeOptions, async2) {
  this.testFileWriting(filePath);
  return new File(filePath, writeOptions, async2);
};
FileRegistry$1.prototype.emitError = function(error, file2) {
  this.emit("error", error, file2);
};
FileRegistry$1.prototype.testFileWriting = function(filePath) {
  mkDir(path$5.dirname(filePath));
  fs$4.writeFileSync(filePath, "", { flag: "a" });
};
function mkDir(dirPath) {
  var isNode1012 = Boolean(url$2.fileURLToPath);
  if (isNode1012) {
    fs$4.mkdirSync(dirPath, { recursive: true });
    return true;
  }
  try {
    fs$4.mkdirSync(dirPath);
    return true;
  } catch (error) {
    if (error.code === "ENOENT") {
      return mkDir(path$5.dirname(dirPath)) && mkDir(dirPath);
    }
    try {
      if (fs$4.statSync(dirPath).isDirectory()) {
        return true;
      }
      throw error;
    } catch (e) {
...
function tryReadJsonAt(searchPath) {
  ...
  try {
    searchPath = path$4.join.apply(path$4, arguments);
    var fileName = findUp("package.json", searchPath);
    if (!fileName) {
      return null;
    }
    var json = JSON.parse(fs$3.readFileSync(fileName, "utf8"));
...
function findUp(fileName, cwd) {
  var currentPath = cwd;
  while (true) {
    var parsedPath = path$4.parse(currentPath);
    var root = parsedPath.root;
    var dir = parsedPath.dir;
    if (fs$3.existsSync(path$4.join(currentPath, fileName))) {
      return path$4.resolve(path$4.join(currentPath, fileName));
    }
    if (currentPath === root) {
      return null;
    }
...
var os$1 = require$$2;
var path$3 = path$7;
var electronApi$1 = electronApi$3;
var packageJson = packageJson$1;
var variables$1 = {
  getAppData,
...
function getAppData(platform2) {
  var appData = electronApi$1.getPath("appData");
  if (appData) {
    return appData;
  }
  var home = getHome();
  switch (platform2) {
    case "darwin": {
      return path$3.join(home, "Library/Application Support");
    }
    case "win32": {
      return process.env.APPDATA || path$3.join(home, "AppData/Roaming");
    }
    default: {
      return process.env.XDG_CONFIG_HOME || path$3.join(home, ".config");
    }
  }
}
function getHome() {
...
function getLibraryDefaultDir(platform2, appName) {
  if (platform2 === "darwin") {
    return path$3.join(getHome(), "Library/Logs", appName);
  }
  return path$3.join(getUserData(platform2, appName), "logs");
}
function getLibraryTemplate(platform2) {
  if (platform2 === "darwin") {
    return path$3.join(getHome(), "Library/Logs", "{appName}");
  }
  return path$3.join(getAppData(platform2), "{appName}", "logs");
}
function getNameAndVersion() {
  var name = electronApi$1.getName() || "";
  var version = electronApi$1.getVersion();
...
function getPathVariables(platform2) {
  var nameAndVersion = getNameAndVersion();
  var appName = nameAndVersion.name;
  var appVersion = nameAndVersion.version;
  return {
    appData: getAppData(platform2),
    appName,
    appVersion,
    electronDefaultDir: electronApi$1.getPath("logs"),
    home: getHome(),
    libraryDefaultDir: getLibraryDefaultDir(platform2, appName),
    libraryTemplate: getLibraryTemplate(platform2),
    temp: electronApi$1.getPath("temp") || os$1.tmpdir(),
    userData: getUserData(platform2, appName)
  };
}
function getUserData(platform2, appName) {
  if (electronApi$1.getName() !== appName) {
    return path$3.join(getAppData(platform2), appName);
  }
  return electronApi$1.getPath("userData") || path$3.join(getAppData(platform2), appName);
}
var fs$2 = require$$0$2;
var path$2 = path$7;
var os = require$$2;
var util$3 = require$$1$1;
var transform$2 = transform_1;
var FileRegistry = file$1.FileRegistry;
var variables = variables$1;
...
function fileTransportFactory(electronLog, customRegistry) {
  ...
  if (registry.listenerCount("error") < 1) {
    registry.on("error", function(e, file2) {
  ...
  transport.format = "[{y}-{m}-{d} {h}:{i}:{s}.{ms}] [{level}]{scope} {text}";
  ...
  return transport;
  function transport(message) {
    var file2 = getFile(message);
    var needLogRotation = transport.maxSize > 0 && file2.size > transport.maxSize;
    if (needLogRotation) {
      transport.archiveLog(file2);
      file2.reset();
    }
    var scopeOptions = electronLog.scope.getOptions();
    ...
    var content = transform$2.transform(message, [
      transform$2.removeStyles,
      transform$2.customFormatterFactory(transport.format, false, scopeOptions),
      transform$2.concatFirstStringElements,
      transform$2.toStringFactory(inspectOptions)
    ]);
    file2.writeLine(content);
  }
  function archiveLog(file2) {
    var oldPath = file2.toString();
    var inf = path$2.parse(oldPath);
    try {
  ...
  function readAllLogs(options) {
    var fileFilter = options && typeof options.fileFilter === "function" ? options.fileFilter : function(fileName) {
      return fileName.endsWith(".log");
    };
    var vars = Object.assign({}, pathVariables, {
      fileName: transport.fileName
    });
    var logsPath = path$2.dirname(transport.resolvePath(vars));
    return fs$2.readdirSync(logsPath).map(function(fileName) {
      return path$2.join(logsPath, fileName);
    }).filter(fileFilter).map(function(logPath) {
      try {
        return {
          path: logPath,
          lines: fs$2.readFileSync(logPath, "utf8").split(os.EOL)
        };
      } catch (e) {
        return null;
      }
  ...
  function initDeprecated() {
    ...
    Object.defineProperties(transport, {
      bytesWritten: {
        get: util$3.deprecate(getBytesWritten, "bytesWritten" + isDeprecatedProp)
      },
      file: {
        get: util$3.deprecate(getLogFile, "file" + isDeprecatedProp),
        set: util$3.deprecate(setLogFile, "file" + isDeprecatedProp)
      },
      fileSize: {
        get: util$3.deprecate(getFileSize, "file" + isDeprecatedProp)
      }
    });
    transport.clear = util$3.deprecate(clear, "clear()" + isDeprecatedText);
    transport.findLogPath = util$3.deprecate(
      getLogFile,
      "findLogPath()" + isDeprecatedText
    );
    transport.init = util$3.deprecate(init, "init()" + isDeprecatedText);
...
function ipcTransportFactory(electronLog) {
  ...
  function transport(message) {
    var ipcMessage = Object.assign({}, message, {
      data: transform$1.transform(message, [
        transform$1.toJSON,
        transform$1.maxDepthFactory(3)
      ])
    });
    electronApi.sendIpc(transport.eventId, ipcMessage);
  }
}
var http$2 = require$$0$4;
var https$2 = require$$1$2;
...
function remoteTransportFactory(electronLog) {
  ...
  transport.transformBody = function(body) {
  ...
  function transport(message) {
    ...
    function onError(error) {
      electronLog.logMessageWithTransports(
        {
          data: [
            "electron-log.transports.remote: cannot send HTTP request to " + transport.url,
            error
          ],
          level: "warn"
        },
...
function post(serverUrl, requestOptions, body) {
  var urlObject = url$1.parse(serverUrl);
  var httpTransport = urlObject.protocol === "https:" ? https$2 : http$2;
  var options = {
...
(function(module) {
  var catchErrors$1 = catchErrors;
  var electronApi2 = electronApi$3;
  var log2 = log_1;
  var scopeFactory2 = scope;
  var transportConsole = consoleExports;
  var transportFile = file;
  var transportIpc = ipc;
  var transportRemote = remote;
  ...
  function create(logId) {
    ...
    instance.scope = scopeFactory2(instance);
    instance.transports = {
    ...
    Object.defineProperty(instance.levels, "add", {
      ...
      value: function add(name, index) {
        index = index === void 0 ? instance.levels.length : index;
        instance.levels.splice(index, 0, name);
        instance[name] = log2.log.bind(null, instance, { level: name });
    ...
    instance.log = log2.log.bind(null, instance, { level: "info" });
    instance.functions.log = instance.log;
    instance.logMessageWithTransports = function logMessageWithTransports(message, transports) {
      ...
      return log2.runTransports(transports, message, instance);
    };
    return instance;
  }
})(src$1);
...
let events$1 = require$$0$1;
let fs$1 = require$$0$2;
let path$1 = path$7;
class devNull {
  info() {
  }
...
class Tail extends events$1.EventEmitter {
  constructor(filename, options = {}) {
    super();
    this.filename = filename;
    this.absPath = path$1.dirname(this.filename);
    ...
    this.nLines = options.nLines || void 0;
    this.logger.info(`Tail starting...`);
    this.logger.info(`filename: ${this.filename}`);
    this.logger.info(`encoding: ${this.encoding}`);
    try {
      fs$1.accessSync(this.filename, fs$1.constants.F_OK);
    } catch (err) {
      if (err.code == "ENOENT") {
        throw err;
      }
    ...
    this.buffer = "";
    this.internalDispatcher = new events$1.EventEmitter();
    this.queue = [];
    ...
    if (fromBeginning) {
      ...
      cursor = 0;
    } else if (this.nLines !== void 0) {
      cursor = this.getPositionAtNthLine(this.nLines);
    } else {
      cursor = this.latestPosition();
    }
    if (cursor === void 0) throw new Error("Tail can't initialize.");
    const flush = fromBeginning || this.nLines != void 0;
    try {
  ...
  getPositionAtNthLine(nLines) {
    const { size } = fs$1.statSync(this.filename);
    if (size === 0) {
      return 0;
    }
    const fd = fs$1.openSync(this.filename, "r");
    ...
    while (lineBytes.length < nLines) {
      ...
      const bytesRead = fs$1.readSync(
        fd,
        buffer,
        0,
        // position in buffer to write to
        chunkSizeBytes,
        // number of bytes to read
        currentReadPosition
        // position in file to read from
      );
  ...
  latestPosition() {
    try {
      return fs$1.statSync(this.filename).size;
    } catch (err) {
  ...
  readBlock() {
    if (this.queue.length >= 1) {
      ...
      if (block.end > block.start) {
        let stream2 = fs$1.createReadStream(this.filename, { start: block.start, end: block.end - 1, encoding: this.encoding });
  ...
  watch(startingCursor, flush) {
    ...
    this.logger.info(`filesystem.watch present? ${fs$1.watch != void 0}`);
    this.logger.info(`useWatchFile: ${this.useWatchFile}`);
    this.isWatching = true;
    this.currentCursorPos = startingCursor;
    if (flush) this.change();
    if (!this.useWatchFile && fs$1.watch) {
      this.logger.info(`watch strategy: watch`);
      this.watcher = fs$1.watch(this.filename, this.fsWatchOptions, (e, filename) => {
        this.watchEvent(e, filename);
      });
    } else {
      this.logger.info(`watch strategy: watchFile`);
      fs$1.watchFile(this.filename, this.fsWatchOptions, (curr, prev) => {
  ...
  rename(filename) {
    if (filename === void 0 || filename !== this.filename) {
      this.unwatch();
      if (this.follow) {
        this.filename = path$1.join(this.absPath, filename);
  ...
  unwatch() {
    if (this.watcher) {
      ...
      fs$1.unwatchFile(this.filename);
    }
    if (this.rewatchId) {
...
const httpAdapter = isHttpAdapterSupported && function httpAdapter2(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    const emitter = new require$$0$1.EventEmitter();
    const onFinished = () => {
    ...
    if (config.transport) {
      ...
      transport = isHttpsRequest ? require$$1$2 : require$$0$4;
    } else {
...
function formatProjectData(response) {
  const numProjects = response.count;
...
function makeRequestToLS(requestJSON2, requestType) {
  const jsonOBJ = {
    ok: null,
...
function formatTextData(textData) {
  const regex = /(?:\([^()]*\)|\d+\.\d+|[^.?!])+[.?!]/g;
  const trimmedRawData = textData.trim();
  if (trimmedRawData && trimmedRawData !== "") {
    const formattedData = [];
    const sentArr = trimmedRawData.match(regex);
    if (sentArr && sentArr.length > 0) {
      sentArr.forEach((sent) => {
        const sentStr = sent.trim();
        if (sentStr !== "") {
          formattedData.push({ text: sentStr });
        }
      });
    } else {
      formattedData.push({ text: trimmedRawData });
    }
...
function formatDataArr(dataArr) {
  ...
  if (dataArr !== null) {
    ...
    for (let i = 0; i < dataArr.length; i++) {
      const data = formatTextData(dataArr[i].textData);
...
function requestJSON(rawData, projectID) {
  const formattedData = formatDataArr(rawData);
...
function exportDataToLS(rawData, projectID) {
  try {
    const request = requestJSON(rawData, projectID);
...
function updateLinkedLSProject(url2) {
  BASEURL = url2;
  APITOKEN = "";
}
...
function logInfo(msg) {
  writeNewLog(msg, "info");
}
function logDebug(msg) {
  writeNewLog(msg, "debug");
}
function logWarn(msg) {
  writeNewLog(msg, "warn");
}
function logError(msg) {
  writeNewLog(msg, "error");
}
...
electron$1.ipcMain.on("log-info", (event, message) => {
  logInfo(`Renderer: ${message}`);
});
electron$1.ipcMain.on("log-debug", (event, message) => {
  logDebug(`Renderer: ${message}`);
});
electron$1.ipcMain.on("log-warn", (event, message) => {
  logWarn(`Renderer: ${message}`);
});
electron$1.ipcMain.on("log-error", (event, message) => {
  logError(`Renderer: ${message}`);
});
electron$1.ipcMain.handle("get-logs", async () => {
  ...
  startLoggingInterval(5e3);
  return logData;
});
electron$1.ipcMain.on("logs:clear", (event) => {
  const logFilePath = log.transports.file.getFile().path;
  try {
    terminateLogListener();
    require$$0$2.writeFileSync(logFilePath, "");
...
function createMainWindow() {
  ...
  const preloadPath = isDev ? path$7.join(__dirname, "../preload/index.js") : path$7.join(__dirname, "../preload/index.js");
  mainWin = new electron$1.BrowserWindow({
    frame: false,
    transparent: true,
  ...
  if (isDev) {
    mainWin.loadURL("http://localhost:5173/").catch((err) => {
      ...
      mainWin.loadFile(path$7.join(__dirname, "../../dist/index.html"));
    });
...
function createURLWindow(url2) {
  try {
    new URL(url2);
  } catch (err) {
    logError(`Invalid URL: ${url2}`);
    return;
  }
  logDebug(`Creating URL window for: ${url2}`);
  urlWindow = new electron$1.BrowserWindow({
    frame: false,
    width: 1400,
    height: 1e3,
    minWidth: 1200,
  ...
  urlWindow.hide();
  urlWindow.loadFile("./renderer/window-templates/scrape-window.html").then(() => {
    urlWindow.webContents.send("setUrl", url2);
    urlWindow.show();
    logInfo(`URL window loaded: ${url2}`);
  }).catch((error) => {
    closeScrapeWindow();
    logError(`Failed to load URL: ${error}`);
    electron$1.dialog.showErrorBox("Invalid URL", "Cannot open URL: the URL you entered was invalid!");
  });
...
function createLSExternal(url2) {
  lsWindow = new electron$1.BrowserWindow({
    frame: false,
    width: 1400,
    height: 1e3,
    minWidth: 1200,
  ...
  try {
    lsWindow.loadFile("./renderer/window-templates/anno-window.html").then(() => {
      lsWindow.webContents.send("set-ls-url", url2);
      lsWindow.show();
    }).catch((err) => {
      closeLSWindow();
      logError("Failed to open external LS window");
      electron$1.dialog.showErrorBox("Failed to open external LS window", "Window Initialization Error");
    });
    lsWindow.on("close", () => {
...
function closeLSWindow(url2 = null) {
  if (lsWindow) {
    lsWindow.close();
    lsWindow = null;
  }
  mainWin.webContents.send("ext-ls-win-closed", url2);
}
function closeAllWindows() {
  closeScrapeWindow();
  closeLSWindow();
}
electron$1.ipcMain.on("open-url", (event, url2) => {
  if (!urlWindow) {
    logDebug(`Received 'open-url' event for URL: ${url2}`);
    try {
      createURLWindow(url2);
    } catch (error) {
...
electron$1.ipcMain.on("open-ls-ext:request", (event, url2) => {
  createLSExternal(url2);
});
electron$1.ipcMain.on("close-scrape-win", (event) => {
  closeScrapeWindow();
});
electron$1.ipcMain.on("ext-ls-url-change", (event, url2) => {
  mainWin.webContents.send("ls-navigation-update", url2);
});
electron$1.ipcMain.on("close-anno-win", (event) => {
  closeLSWindow();
});
...
electron$1.ipcMain.on("exit:request", () => {
  logInfo("Received exit request from renderer.");
  terminateApp();
});
electron$1.ipcMain.on("export-to-ls:request", async (event, data, projectID) => {
...
electron$1.ipcMain.on("init-ls-vars:request", (event, url2, token) => {
  updateLinkedLSProject(url2);
  updateAPIToken(token).then((result) => {
    mainWin.webContents.send("ls-projects-update", JSON.stringify(result));
  }).catch((err) => {
    console.log(err);
  });
});
electron$1.ipcMain.on("update-linked-ls:request", (event, url2) => {
  updateLinkedLSProject(url2);
});
electron$1.ipcMain.on("update-ls-api-token:request", (event, token) => {
  updateAPIToken(token).then((result) => {
    mainWin.webContents.send("ls-projects-update", JSON.stringify(result));
  }).catch((err) => {
    console.log(err);
...
electron$1.ipcMain.on("clear-linked-ls:request", () => {
  const res = clearLinkedLSProject();
  mainWin.webContents.send("ls-projects-update", JSON.stringify(res));
});
electron$1.ipcMain.on("scrapedData:export", (event, data) => {
  if (mainWin) {
...
electron$1.ipcMain.on("gen-dialog", (event, message) => {
  const json = JSON.parse(message);
  electron$1.dialog.showMessageBox({ message: json.msg });
});
electron$1.ipcMain.on("err-dialog", (event, message) => {
  const json = JSON.parse(message);
  electron$1.dialog.showErrorBox(json.errType, json.msg);
});
